...
export async function generateMetadata({
  ...
  const decodedGenre = decodeURIComponent(params.slug);
  const page = Math.max(1, parseInt(searchParams.page ?? "1", 10));
  const query = searchParams.q?.trim() || undefined;
  const { total } = await getMovies(1, 1, { genre: decodedGenre, query });
  const pageSuffix = page > 1 ? ` - Page ${page}` : "";
  const searchSuffix = query ? ` matching "${query}"` : "";
  const title = `${decodedGenre} Movies${searchSuffix}${pageSuffix}`;
  const description = compactText(
    `Browse ${total} ${decodedGenre} movie${total === 1 ? "" : "s"} and series on Movies I Loved.${query ? ` Searching for "${query}".` : ""}${page > 1 ? ` Page ${page}.` : ""}`
  );
  const canonical = absolutePublicUrl(
    `/genre/${encodeURIComponent(decodedGenre)}${query || page > 1 ? "?" : ""}${query ? `q=${encodeURIComponent(query)}` : ""}${query && page > 1 ? "&" : ""}${page > 1 ? `page=${page}` : ""}`
  );
...
export default async function GenrePage({
  ...
  const [{ movies, total }, monetizationConfig, facetStats] = await Promise.all([
    getMovies(page, PAGE_SIZE, { genre: decodedGenre, query }),
  ...
  const archiveSearchBase = `/genre/${encodeURIComponent(decodedGenre)}`;
  ...
  const jsonLd = {
    ...
    "@type": "CollectionPage",
    name: `${decodedGenre} Movies`,
    description: compactText(`Browse ${total} ${decodedGenre} movies and series on Movies I Loved.`),
    url: pageUrl,
    mainEntity: {
      "@type": "ItemList",
      ...
      <div className="mx-auto w-full max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col gap-4 rounded-[1.4rem] border border-[#222222] bg-[#111111] p-5">
          <div>
            ...
            <h2 className="mt-2 text-3xl font-black text-emerald-400 capitalize">{decodedGenre} Movies</h2>
        ...
        <ArchiveSearchPanel
          title="Search within this genre"
          description={`Search the ${decodedGenre} archive without leaving the page.`}
          baseHref={`/genre/${encodeURIComponent(decodedGenre)}`}
          currentQuery={query}
          currentLabel={`Genre: ${decodedGenre}`}
          total={total}
          resultLabel={`${decodedGenre} titles`}
        />

        <SuggestedSearchesRow
          title="Suggested searches"
          description="Quick shortcuts that keep you in this genre while narrowing by common release terms."
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <FacetRail
            title="More genres"
            description="Popular category jumps across the catalog."
            currentValue={decodedGenre}
            items={facetStats.genres}
            buildHref={(value) => `/genre/${encodeURIComponent(value)}`}
          />
          <FacetRail
            title="Popular languages"
            description="Jump into the most common audio and language buckets."
            items={facetStats.languages}
            buildHref={(value) => `/language/${encodeURIComponent(value)}`}
          />
          <FacetRail
            title="Browse years"
            description="Move sideways into the most active release years."
        ...
        {movies.length > 0 ? (
          ...
          <div className="flex h-64 flex-col items-center justify-center gap-4 rounded-[1.4rem] border border-[#222222] bg-[#111111]">
            <p className="text-sm text-zinc-600">No movies found in this genre.</p>
